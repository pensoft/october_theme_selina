...
function setZenodoCookie(name, value, days){
    var expires = "";
    if (days) {
        var date = new Date();
...
function openZenodoModalBtn(){
    // Don't display the popup again once it has been closed
    if (getZenodoCookie("zenodoPopupClosed")) {
        return;
    }
    ...
    // When the popup is closed, set a cookie so it won't be shown anymore
    $('#content-confirmation').on('hidden.bs.modal', function() {
        setZenodoCookie("zenodoPopupClosed", "1", 365);
    });
}
